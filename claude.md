...
18. **에디터 이미지 붙여넣기/드롭** (2026-07-13 완료, 사용자 선택: base64 인라인) — `SourcePane`(CM6)에 `CMEditorView.domEventHandlers({ paste, drop })` 확장 추가(`insertImageFile` 헬퍼). 클립보드/드래그의 이미지 파일을 `FileReader.readAsDataURL` → `![이미지](data:image/…)` 로 커서(또는 `posAtCoords`) 위치 삽입, 이미지 없으면 `return false`로 기본 동작 유지(텍스트 붙여넣기·사이드바 노트 DnD 무간섭). 2MB 초과 거부(`MAX_PASTE_IMAGE_BYTES` — localStorage ~5MB·노트 md 페이로드 보호), 토스트는 `onToast` prop→`onToastRef`(extensions 1회 생성 패턴). **★함정: react-markdown v9 `defaultUrlTransform`이 `data:` URI를 제거**해 프리뷰 이미지가 조용히 깨짐 → `MarkdownPreview`에 `urlTransform={(url)=>url.startsWith('data:image/')?url:defaultUrlTransform(url)}` 필수. 동기화·발행은 base64가 md에 그대로 들어가 추가 작업 없음(단 이미지 많으면 페이로드 큼). E2E: 실제 클립보드 write(ClipboardItem)+Ctrl+V로 검증.
